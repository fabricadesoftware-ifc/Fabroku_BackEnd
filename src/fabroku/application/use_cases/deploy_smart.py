...
class SmartDeployUseCase:
	...
	def execute(
		...
		if not app_name or not git_url:
			return OperationResult(False, "'app_name' e 'git_url' são obrigatórios para smart-deploy.")

		try:
			if state_sync:
				state_sync.set_status("em_andamento")
				state_sync.append_log("Iniciando análise do repositório...")
			...
			repo_url = git_url
			if "#" in git_url:
				repo_url, branch = git_url.split("#", 1)
			...
			if state_sync:
				state_sync.append_log("Executando deploy via Dokku (git:sync)...")
			...
			# Falhou
			if state_sync:
				analysis["strategy"] = analysis.get("strategy") or "dokku-git"
				state_sync.set_analysis(analysis)
