...
from applications.github_integration import ensure_github_webhook
from applications.models import App, AppProcessScale, AppRunArtifact, AppRunArtifactKind
from applications.process_scale import get_process_max_instances, sync_app_process_scales_from_dokku
...
from infrastructure.adapters.git_utils import get_github_hook_events, normalize_webhook_url, parse_github_repo_name
from infrastructure.cache_versioning import APP_LAST_COMMIT_CACHE_NAMESPACE, build_versioned_cache_key, get_cache_ttl
...
from .serializers import (
    AppProcessScaleSerializer,
    AppSerializer,
    EnvVarsUpdateSerializer,
    InteractiveSessionAnswerSerializer,
    InteractiveSessionCreateSerializer,
    InteractiveSessionInputSerializer,
    ScaleProcessesSerializer,
)
...
class AppViewSet(ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        task_result = delete_app_task.delay(app_id=instance.id, deleted_by_id=request.user.id)
    ...
    def start(self, request, pk=None):
        ...
        task_result = manage_app_task.delay(app_id=app.id, action='start')
    ...
    def stop(self, request, pk=None):
        ...
        task_result = manage_app_task.delay(app_id=app.id, action='stop')
    ...
    def restart(self, request, pk=None):
        ...
        task_result = manage_app_task.delay(app_id=app.id, action='restart')
    ...
    def scale_processes(self, request, pk=None):
        ...
        task_result = scale_processes_task.delay(app_id=app.id, processes=processes)
    ...
    def redeploy(self, request, pk=None):
        ...
        task_result = deploy_app_task.delay(
            app_id=app.id,
            commit=commit,
    ...
    def run_command(self, request, pk=None):
        ...
        task_result = run_command_task.delay(app_id=app.id, command=command)
    ...
    def run_migrate(self, request, pk=None):
        ...
        task_result = run_migrate_task.delay(
            app_id=app.id,
    ...
    def run_loaddata(self, request, pk=None):
        ...
        cleanup_expired_run_artifacts()
        task_result = run_loaddata_task.delay(
            app_id=app.id,
    ...
    def run_dumpdata(self, request, pk=None):
        ...
        cleanup_expired_run_artifacts()
        task_result = run_dumpdata_task.delay(
            app_id=app.id,
            manage_path=manage_path,
            dump_args=dump_args,
