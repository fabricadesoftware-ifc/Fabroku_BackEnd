...
from observability.models import AppLog, SSHCommandAudit
from projects.models import Project

from .ssh_audit import begin_ssh_audit, finish_ssh_audit, ssh_audit_context
...
class AppLogVisibilityTests(APITestCase):
    ...
    @patch('observability.views.has_live_runner', return_value=True)
    @patch('observability.views.remove_subscriber_async', new_callable=AsyncMock)
    @patch('observability.views.touch_subscriber_async', new_callable=AsyncMock)
    @patch('observability.views.get_async_logstream_redis')
    @patch('observability.views.get_logstream_redis')
    @patch('observability.views.read_buffer_async')
    ...
    @patch('observability.views.has_live_runner', return_value=False)
    @patch('observability.views.get_logstream_redis')
